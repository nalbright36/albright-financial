...
def get_news_sentiment_for_symbol(symbol):
    """
    Returns a news sentiment dict for one symbol, built from
    NewsSentimentSummary — parallel structure to
    get_sentiment_for_symbol, same safe-default behavior when the
    scheduled news command hasn't produced a row for this symbol yet.
    """
    from .models import NewsSentimentSummary

    summary = NewsSentimentSummary.objects.filter(symbol=symbol).first()
